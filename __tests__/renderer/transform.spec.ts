...
describe('transform', () => {
  test('Renderer should apply specified transforms and can save or restore transforms context.', () => {
    ...
    expect((r1.parentNode as Element).getAttribute('transform')).toBe('translate(200, 100) rotate(60) scale(2, 3)');
    expect((r2.parentNode as Element).getAttribute('transform')).toBeNull();
  });
});
