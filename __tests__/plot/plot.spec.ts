...
describe('plot', () => {
  ...
  test('symmetried interval', () => {
    const chart = sp.plot({
      ...
      data: sports,
      paddingLeft: 60,
      transforms: [(data: any) => data.sort((a: any, b: any) => b.sold - a.sold)],
  ...
  test('stacked interval in polar', () => {
    const chart = sp.plot({
      ...
      transforms: [
        (data: any) => {
          const sum = data.reduce((total: number, d: any) => total + d.sold, 0);
          return data.map(({ genre, sold }: { genre: any, sold: number }) => ({ genre, sold: sold / sum }));
        },
      ],
  ...
  test('line', () => {
    const chart = sp.plot({
      ...
      guides: {
        x: { formatter: (d:any) => d.getFullYear(), grid: true },
        y: { grid: true },
      },
      encodings: {
        x: 'year',
  ...
  test('point with interpolated log scale', () => {
    const chart = sp.plot({
      ...
      scales: {
        ...
        r: {
          range: [3, 20],
          interpolate: (t:any, a:any, b:any) => Math.sqrt(a * a * (1 - t) + b * b * t),
        },
      },
      guides: {
        y: { formatter: (d:any) => d | 0 },
      },
      encodings: {
        x: 'year',
        y: 'value',
  ...
  test('link', () => {
    const chart = sp.plot({
      ...
      data: profit,
      paddingLeft: 80,
      transforms: [(data:any) => data.filter((d:any) => d.month !== 'Total')],
      scales: {
        color: { type: 'identity' },
        y: { nice: true },
      },
  ...
  test.skip('ribbon', () => {
    fetch(
      ...
      const chart = sp.plot({
        ...
        guides: {
          color: { width: 80, formatter: (d:any) => d.split(' ').pop() },
          y: { formatter: (d:any) => `${(d / 1000) | 0}k` },
        },
        children: [
          {
            type: 'area',
