import type { ComputeFacetViews } from "./interface.d";
import { group } from '../utils';

const computeFacetViews: ComputeFacetViews = (box, {
  data, encodings, padding = 0,
  paddingLeft = 45, paddingRight = 45, paddingBottom = 45, paddingTop = 60,
}) => {
  const { x, y } = encodings;
  const cols = x ? Array.from(group(data, (d: any) => d[x]).keys()) : [undefined];
  const rows = y ? Array.from(group(data, (d: any) => d[y]).keys()) : [undefined];
  const n = cols.length;
  const m = rows.length;
  const views = [];
  ...
  for (let i = 0; i < n; i += 1) {
    for (let j = 0; j < m; j += 1) {
      const transform = (data: any[]) => {
        const inRow = (d: any) => d[x] === cols[i] || cols[i] === undefined;
        const inCol = (d: any) => d[y] === rows[j] || rows[j] === undefined;
